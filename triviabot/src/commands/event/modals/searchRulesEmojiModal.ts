...
export default commandModule({
  ...
  name: "search_rules_emoji", // must match your modal’s customId
  async execute(ctx: ModalSubmitInteraction) {
    try {
      ...
      const term = ctx.fields.getTextInputValue("rEmoji_search").toLowerCase();
      const matches = ctx
      ...
      const menu = new StringSelectMenuBuilder()
        .setCustomId("event_r_emoji_select")
        .setPlaceholder("Pick rules emoji…")
        .addOptions(...opts);
