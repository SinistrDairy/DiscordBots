...
export default commandModule({
  ...
  execute: async (ctx) => {
    ...
    if (userProfile) {
      const landName = userProfile.land.split(' ').map((word: any) =>  {return (word.charAt(0).toUpperCase() + word.slice(1))}).join(' ')
      if (adjustment === 1) {
        ...
        // console.log(`Added ${jewels} to ${userProfile.land}`)
        ...
        // console.log(`Subtracted ${jewels} from ${userProfile.land}`)
        const channel = ctx.client.channels.cache.get('1374744395563270205') as TextChannel
