...
export default commandModule({
  ...
  options: [
    ...
    { type: ApplicationCommandOptionType.Number, name: "agrabah", description: "Jewels for Agrabah", required: true },
    { type: ApplicationCommandOptionType.Number, name: "hundred_acre_wood", description: "Jewels for Hundred Acre Wood", required: true },
    { type: ApplicationCommandOptionType.Number, name: "monstropolis", description: "Jewels for Monstropolis", required: true },
  ],
  ...
  execute: async (ctx) => {
    ...
    try {
      ...
      const inputs = [
        ...
        { name: "Hundred Acre Wood", jewels: ctx.options.getNumber("hundred_acre_wood", true) },
        { name: "Monstropolis", jewels: ctx.options.getNumber("monstropolis", true) },
      ];

      // 2) Fetch all land docs in one query
      const landDocs = await landsSchema.find(
        { name: { $in: inputs.map(i => i.name) } },
        "name emojiID"
      ).lean();
      ...
      if (logChannel?.isTextBased()) {
        for (const { name, jewels } of inputs) {
      ...
      ["1374744395563270205", "1220081937906008144"].forEach(id => {
