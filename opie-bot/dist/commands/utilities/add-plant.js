...
var add_plant_default = commandModule({
  name: "add-plants",
  ...
  execute: async (ctx) => {
    const target = ctx.member;
    const roleIDs = /* @__PURE__ */ new Set(["1367191933335765032", "1364313155668410562"]);
    const roleCheck = target.roles.cache.filter(
      (role) => roleIDs.has(role.id)
    );
    const hasRoles = roleCheck.size > 0;
    console.log(hasRoles);
    if (hasRoles === false) {
      const noRole = new EmbedBuilder().setTitle("__No Subscription Found__").setColor("Red").setDescription(
